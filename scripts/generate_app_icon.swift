...
func drawIcon(pixels: Int, to url: URL) throws {
    ...
    let fieldRect = rect.insetBy(dx: size.width * 0.11, dy: size.height * 0.11)
    ...
    let postWidth = size.width * 0.07
    let postHeight = size.height * 0.36
    let postY = size.height * 0.32
    for x in [fieldRect.minX + size.width * 0.04, fieldRect.maxX - size.width * 0.11] {
        let post = NSBezierPath(roundedRect: NSRect(x: x, y: postY, width: postWidth, height: postHeight), xRadius: postWidth * 0.45, yRadius: postWidth * 0.45)
